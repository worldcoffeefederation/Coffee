...
CFE Masternode installer arguments:
...
# Set these to change the version of coffee to install
TARBALLURL="https://github.com/worldcoffeefederation/Coffee/releases/download/1.1.0/coffee-1.1.0-x86_64-linux-gnu.tar.gz"
TARBALLNAME="coffee-1.1.0-x86_64-linux-gnu.tar.gz"
BOOTSTRAPURL=""
BOOTSTRAPARCHIVE=""
BWKVERSION="1.1.0"
...
echo "
 ...
 | the CFE Team CANNOT assist you in repairing  |::
...
echo "" && echo 'Added user "coffee"' && echo ""
sleep 1
...
rm $TARBALLNAME
mv ./coffeed /usr/local/bin
mv ./coffee-cli /usr/local/bin
mv ./coffee-tx /usr/local/bin
rm -rf $TARBALLNAME

# Create .coffee directory
mkdir $USERHOME/.coffee
...
# Create coffee.conf
touch $USERHOME/.coffee/coffee.conf
cat > $USERHOME/.coffee/coffee.conf << EOL
${INSTALLERUSED}
rpcuser=${RPCUSER}
...
masternode=1
addnode=167.179.65.223
addnode=202.182.118.110
EOL
chmod 0600 $USERHOME/.coffee/coffee.conf
chown -R $USER:$USER $USERHOME/.coffee

sleep 1

cat > /etc/systemd/system/coffee.service << EOL
[Unit]
Description=coffeed
...
ExecStart=/usr/local/bin/coffeed -conf=${USERHOME}/.coffee/coffee.conf -datadir=${USERHOME}/.coffee
ExecStop=/usr/local/bin/coffee-cli -conf=${USERHOME}/.coffee/coffee.conf -datadir=${USERHOME}/.coffee stop
Restart=on-abort
[Install]
